...
import {fetchTaskDailyTelegrams, fetchTaskDailyTelegramsCompletedByUser, fetchTaskDailyTelegramsCompleted} from "@/Database/getAPI";
...
const TaskDailyGet = () => {
    ...
    useEffect(() => {
        const getTaskDailyTelegrams = async () => {
            try {
                const data = await fetchTaskDailyTelegrams(Token);
        ...
        getTaskDailyTelegrams();
    }, [Token]);
    useEffect(() => {
        const getTaskTelegramsCompletedByUser = async () => {
            try {
                if (taskTelegrams.length > 0) {
                    const data = await fetchTaskDailyTelegramsCompletedByUser(Token);
                                            ...
                                            <button className={`buttonz size-nc bg-opacity-25`} 
                                                onClick={ () => { 
                                                    ...
                                                    new Promise(resolve => setTimeout(resolve, 10000)).then(() => {
                                                        fetchTaskDailyTelegramsCompleted(task.id, Token);
